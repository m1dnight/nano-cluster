...
%% Vendored from atomvm_lib (src/httpd.erl @ master) so nano_cluster can pack it
%% into the app image - httpd is not part of the flashed AtomVM stdlib. The only
%% change from upstream: the trace.hrl include_lib (which needs the atomvm_lib
%% app on the path) is a plain -include of the copy sitting next to this file.
%% See also src/gen_tcp_server.erl, src/httpd.hrl, src/trace.hrl.
